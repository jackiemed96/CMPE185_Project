...
class Yolov7Publisher:
    def __init__(self, node:Node, img_topic: str, weights: str, conf_thresh: float = 0.5,
        ...
        self.visualization_publisher = Node.create_publisher(
            node, topic=vis_topic, msg_type=Image, qos_profile=10
        ...
        self.img_subscriber = Node.create_subscription(
            node, topic=img_topic, msg_type=Image, callback=self.process_img_msg, qos_profile=10
        )
        self.detection_publisher = Node.create_publisher(
            node, topic=pub_topic, msg_type=Detection2DArray, qos_profile=10
        )
        #rclpy.loginfo("YOLOv7 initialization complete. Ready to start inference")
    ...
    def process_img_msg(self, img_msg: Image):
        ...
        # publishing
        detection_msg = create_detection_msg(self.node, img_msg, detections)
...
if __name__ == "__main__":
    rclpy.init()
    node = rclpy.create_node("objdetect_node")

    #ns = node.get_name() + "/"
    ns = ""

    node.declare_parameter(ns + "weights_path")
    node.declare_parameter(ns + "classes_path")
    node.declare_parameter(ns + "img_topic")
    node.declare_parameter(ns + "out_topic")
    node.declare_parameter(ns + "conf_thresh")
    node.declare_parameter(ns + "iou_thresh")
    node.declare_parameter(ns + "queue_size")
    node.declare_parameter(ns + "img_size")
    node.declare_parameter(ns + "visualize")
    node.declare_parameter(ns + "device")

    weights_path = node.get_parameter(ns + "weights_path")
    classes_path = node.get_parameter(ns + "classes_path")
    img_topic = node.get_parameter(ns + "img_topic")
    out_topic = node.get_parameter(ns + "out_topic")
    conf_thresh = node.get_parameter(ns + "conf_thresh")
    iou_thresh = node.get_parameter(ns + "iou_thresh")
    queue_size = node.get_parameter(ns + "queue_size")
    img_size = node.get_parameter(ns + "img_size")
    visualize = node.get_parameter(ns + "visualize")
    device = node.get_parameter(ns + "device")

    # some sanity checks
    if not os.path.isfile(weights_path.value):
        raise FileExistsError(f"Weights not found ({weights_path.value}).")
    
    if classes_path: 
        if not os.path.isfile(classes_path.value):
            raise FileExistsError(f"Classes file not found ({classes_path.value}).")
        classes = parse_classes_file(classes_path.value)
    else:
        rclpy.loginfo("No class file provided. Class labels will not be visualized.")
        classes = None

    if not ("cuda" in device.value or "cpu" in device.value):
        raise ValueError("Check your device.")


    publisher = Yolov7Publisher(
        node=node,
        img_topic=img_topic.value,
        pub_topic=out_topic.value,
        weights=weights_path.value,
        device=device.value,
        visualize=visualize.value,
        conf_thresh=conf_thresh.value,
        iou_thresh=iou_thresh.value,
        img_size=(img_size.value, img_size.value),
